...
# normalization 87.6 --> 87.8 (512*512*3)
# pretrain imagenet --> noisy-student 87.82
# opt.swa --> 87.85
# + fc 1024*1024, with 0.2 dropout 87.90(+training time)
# + channel shuffle 87.92
# + pesudo label 88.02~
# + embedding ~88.12
# mixup
# timeshift ~88.25 -->88.08
...
gcs_paths = []
for i in[0,1,2,3]:
    GCS_path = KaggleDatasets().get_gcs_path(f"19970811-face-generation-{i}")
    gcs_paths.append(GCS_path)
    print(GCS_path)
gcs_test_paths = []
for i in [0,1]:
    GCS_path = KaggleDatasets().get_gcs_path(f"19970811-face-generations-{i}")
    gcs_test_paths.append(GCS_path)
    gcs_paths.append(GCS_path)
    print(GCS_path)
TRAINING_FILENAMES = []
for path in gcs_paths:
    TRAINING_FILENAMES.extend(np.sort(np.array(tf.io.gfile.glob(path + "/train*.tfrecords"))))
    #TRAINING_FILENAMES.extend(np.sort(np.array(tf.io.gfile.glob(path + "/test*.tfrecords"))))
print("train_files: ", len(TRAINING_FILENAMES))

TESTING_FILENAMES = []
for path in gcs_test_paths:
    TESTING_FILENAMES.extend(np.sort(np.array(tf.io.gfile.glob(path + "/test*.tfrecords"))))
print("test_files: ", len(TESTING_FILENAMES))
...
# TRAINING_FILENAMES = tf.io.gfile.glob(GCS_PATH1 + '/train*.tfrec') + tf.io.gfile.glob(GCS_PATH2 + '/train*.tfrec') + tf.io.gfile.glob(GCS_PATH3 + '/train*.tfrec') + tf.io.gfile.glob(GCS_PATH4 + '/test*.tfrec') + tf.io.gfile.glob(GCS_PATH5 + '/test*.tfrec')
...
# target_semi = pd.read_csv("../input/g2net-new-ensemble/submission.csv")
# # train_semi = pd.read_csv("../input/g2net-gravitational-wave-detection/training_labels.csv")
# # merge_semi = pd.concat([target_semi, train_semi])
...
def prepare_image(wave):
    ...
    normalized_waves = []
    mean = tf.constant([6.90712e-26, 5.1178e-26, 1.38318e-26], dtype=tf.float64)
    std = tf.constant([7.419e-21, 7.420e-21, 1.838e-21], dtype=tf.float64)
    global_mean = tf.constant(3.5452e-26, dtype=tf.float64)
    global_std = tf.constant(6.15079e-21, dtype=tf.float64)

    # Normalize
    for i in range(3):
#         normalized_wave = (wave[i]-mean[i]) / std[i]
        normalized_wave = (wave[i]-global_mean) / global_std
#         normalized_wave = wave[i] / tf.math.reduce_max(wave[i])
    ...
    # Create image
    image = create_cqt_image(wave, HOP_LENGTH) # 64*513*3
    # Resize image
    image = tf.image.resize(image, [*IMAGE_SIZE]) #3 * 512*512
    # Reshape
    image = tf.reshape(image, [*IMAGE_SIZE, 3])
    return image

def channel_shuffel(image, p=0.5):
    if random.randint(0, 1) < p:
        return tf.transpose(tf.random.shuffle(tf.transpose(image, [2, 0, 1])), [1,2,0])
    else: return image

def prepare_target(target):
    return tf.io.decode_raw(target, tf.float64)
...
def read_labeled_tfrecord(example):
    ...
    #try:
    example = tf.io.parse_single_example(example, LABELED_TEST_FORMAT)
...
NUM_TRAINING_IMAGES = 786000#count_data_items(TRAINING_FILENAMES)560000
NUM_TESTING_IMAGES = 226000#count_data_items(TESTING_FILENAMES)
